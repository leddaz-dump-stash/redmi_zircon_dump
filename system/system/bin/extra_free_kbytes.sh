...
vendor_watermark_scale_factor=`getprop persist.vendor.watermark_scale_factor`

if [ ${#vendor_watermark_scale_factor} -gt 0 ]
then
    ...
    if [ $RamSizeGB -gt 8 ]; then
        max_watermark_scale=$vendor_watermark_scale_factor
    fi
fi
